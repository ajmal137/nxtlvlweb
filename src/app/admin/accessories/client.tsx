...
export default function AccessoryInventoryClient({
    initialAccessories,
}: {
    initialAccessories: Accessory[];
}) {
    const [items, setItems] = useState<Accessory[]>(initialAccessories);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (editingItem) {
            const { data, error } = await updateAccessory(editingItem.id, submitData);
            if (error) {
                alert("Failed to update accessory: " + error);
            } else if (data) {
                setItems(items.map((i) => (i.id === editingItem.id ? (data as Accessory) : i)));
                setIsDialogOpen(false);
            }
        } else {
            const { data, error } = await createAccessory(submitData);
            if (error) {
                alert("Failed to create accessory: " + error);
            } else if (data) {
                setItems([data as Accessory, ...items]);
                setIsDialogOpen(false);
            }
        }
        setLoading(false);
    ...
    const handleDelete = async (id: string) => {
        if (confirm("Are you sure you want to delete this accessory?")) {
            ...
            if (success) {
                ...
                alert("Failed to delete accessory: " + error);
            }
        }
    };
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-muted-foreground">Manage your accessory listings.</p>
                </div>
                <Button
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogContent className="bg-card border-white/10 sm:max-w-[425px]">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {editingItem
                                ? "Update accessory details below."
                                : "Enter new accessory details below."}
                        </DialogDescription>
                    </DialogHeader>
